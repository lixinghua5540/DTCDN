...
class NasNetA(nn.Module):
    ...
    def __init__(self, repeat_cell_num=4, reduction_num=2, filters=44, stemfilter=44):
        super().__init__()
        ...
        self.l1,self.l2 = self._make_layers(repeat_cell_num, reduction_num)
    ...
    def forward(self, x):
        ...
        prev = None
        p0=x
        x, prev = self.l1((x, prev))
        p1=x
        x, prev = self.l2((x, prev))
        x = self.relu(x)
        p2=x
