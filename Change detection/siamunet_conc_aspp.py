...
from encoder import get_scope_table
...
class _ASPPModule(nn.Module):
    def __init__(self, inplanes, planes, kernel_size, padding, dilation, BatchNorm=nn.BatchNorm2d):
        ...
        self.bn = BatchNorm(planes,eps=BN_EPSILON,momentum=BN_MOMENTUM,track_running_stats=True)
        self.relu = nn.ReLU()
...
class ASPP(nn.Module):
    def __init__(self,encoder_name,BatchNorm=nn.BatchNorm2d):
        super(ASPP, self).__init__()
        inplanes =get_scope_table(encoder_name=encoder_name,poolnum=4,out=True)
        ...
        self.bn1 = BatchNorm(256,eps=BN_EPSILON,momentum=BN_MOMENTUM,track_running_stats=True)
        self.relu = nn.ReLU()
