...
class NICE(object) :
    ...
    def build_model(self):
        ...
        print('[Network %s] Total number of parameters: ' % 'disA', sum(param.numel() for param in self.disA.parameters()))
        ...
        macs, params = clever_format([macs*2, params*2], "%.3f")
        print('[Network %s] Total number of parameters: ' % 'gen2B', sum(param.numel() for param in  self.gen2B.parameters()))
    ...
    def load(self):
        params = torch.load(os.path.join(self.result_dir, self.dataset+'_params_latest.pt'))
        #params = torch.load('E:/code/NICE-GAN-pytorch-master/results/Gloucester/model/Gloucester_params_0290000.pt')
    ...
    def test(self):
        ...
        for n, (real_A, real_A_path) in enumerate(self.testA_loader):
            ...
            print(real_A_path[0][25:])
            strlist = real_A_path[0].split('/')
            name = strlist[-1]
            cv2.imwrite(os.path.join(self.result_dir, self.dataset, 'fakeB', name), A2B * 255.0)
            #cv2.imwrite(os.path.join(self.result_dir, self.dataset, 'fakeB', real_A_path[0][26:]), A2B * 255.0)
            #cv2.imwrite(os.path.join(self.result_dir, self.dataset, 'fakeB', real_A_path[0][23:]), A2B * 255.0)
        ...
        for n, (real_B, real_B_path) in enumerate(self.testB_loader):
            ...
            strlist = real_B_path[0].split('/')
            name = strlist[-1]
            cv2.imwrite(os.path.join(self.result_dir, self.dataset, 'fakeA', name), B2A * 255.0)
            #cv2.imwrite(os.path.join(self.result_dir, self.dataset, 'fakeA', real_B_path[0][26:]), B2A * 255.0)
            #cv2.imwrite(os.path.join(self.result_dir, self.dataset, 'fakeA', real_B_path[0][23:]), B2A * 255.0)
